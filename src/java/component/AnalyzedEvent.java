...
public class AnalyzedEvent 
{
    ...
    private List<ClassTable> classTable = new ArrayList();
    ...
    public AnalyzedEvent(Events e, String driver, String className, String carName, String classPosition, String rawPosition, String paxPosition, List<Object[]> competitorRuns)
    {
        ...
        for(int x = 0; x < competitorRuns.size(); x++)
        {
            if(x == 0)
            {
                classTable.add(new ClassTable(x+1, String.valueOf(competitorRuns.get(x)[1]), String.valueOf(competitorRuns.get(x)[2]), (double)competitorRuns.get(x)[0], 0.000, 0.000));
            ...
            {
                classTable.add(new ClassTable(x+1, String.valueOf(competitorRuns.get(x)[1]), String.valueOf(competitorRuns.get(x)[2]), (double)competitorRuns.get(x)[0], lastTime-(double)competitorRuns.get(x)[0], topTime-(double)competitorRuns.get(x)[0]));
                lastTime = (double)competitorRuns.get(x)[0];
            }
